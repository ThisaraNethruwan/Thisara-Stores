...
// A variant has a label (e.g. "Vanilla") and an optional price_override
...
export default function AdminDashboard() {
  ...
  const handleDeleteOrder = async (id) => {
    ...
    try {
      ...
      console.error('Delete order failed:', e.code, e.message)
  ...
  // ── VARIANT HELPERS ────────────────────────────────────────────────────────
  const addVariant = () => {
    setPForm(f => ({ ...f, variants: [...(f.variants || []), { ...EMPTY_VARIANT }] }))
  }

  const removeVariant = (idx) => {
    setPForm(f => ({ ...f, variants: f.variants.filter((_, i) => i !== idx) }))
  }
  ...
  const saveProduct = async () => {
    ...
    if (pForm.has_variants) {
      if (!pForm.variants || pForm.variants.length < 2) {
        toast.error('Add at least 2 variants (e.g. Vanilla, Chocolate)')
        return
      }
      for (const v of pForm.variants) {
        if (!v.label.trim()) { toast.error('All variant labels must be filled in'); return }
      }
    }

    // Clean variants: remove empty price_override, trim labels
    const cleanedVariants = pForm.has_variants
      ? pForm.variants.map(v => ({
          label: v.label.trim(),
          ...(v.price_override !== '' && !isNaN(Number(v.price_override))
  ...
  const handleToggleProduct = async (id, active) => {
    try {
      ...
      console.error('Toggle product failed:', e)
    ...
    <>
      ...
      <div style={{ padding:'14px 12px', borderTop:'1px solid rgba(255,255,255,.07)', flexShrink:0 }}>
        ...
        <button onClick={handleLogout}
          style={{ width:'100%', background:'rgba(230,57,70,.2)', color:'#f87171', border:'1.5px solid rgba(230,57,70,.3)', padding:'9px', borderRadius:10, fontWeight:700, cursor:'pointer', fontFamily:"'Nunito',sans-serif", fontSize:13, transition:'all .2s' }}
                ...
                <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', borderBottom:'1px solid #f5f5f5', fontSize:13 }}>
                  <span style={{ color:'#444' }}>
                    ...
                    {item.selectedVariant && (
                      <span style={{ background:'#ede9fe', color:'#5b21b6', fontSize:10, fontWeight:700, padding:'1px 7px', borderRadius:50, marginLeft:6 }}>
                        {item.selectedVariant}
                      </span>
                    )}
              ...
              <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
                {['pending','confirmed','delivered','cancelled'].map(s => (
                  <button key={s} onClick={() => handleUpdateStatus(order.id, s)} style={{
                    ...
                    flex:'1 1 auto', transition:'all .15s',
                  }}>
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .del-btn-inline:disabled { opacity:0.6; cursor:not-allowed; }
        ...
        .variants-box-title { font-size:13px; font-weight:800; color:#5b21b6; }
        .variant-row { display:grid; grid-template-columns:1fr 140px 32px; gap:8px; align-items:center; margin-bottom:8px; }
        .variant-inp { padding:9px 12px; border:2px solid #ddd6fe; border-radius:9px; font-size:13px; outline:none; font-family:'Nunito',sans-serif; background:#fff; width:100%; transition:border-color .2s; box-sizing:border-box; }
        .variant-inp:focus { border-color:#7c3aed; }
        .variant-rm-btn { width:32px; height:32px; border-radius:8px; border:2px solid #fca5a5; background:#fff5f5; color:#e63946; font-size:16px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all .15s; flex-shrink:0; }
        .variant-rm-btn:hover { background:#e63946; color:#fff; border-color:#e63946; }
        .add-variant-btn { width:100%; padding:9px; border:2px dashed #c4b5fd; border-radius:10px; background:#fff; color:#7c3aed; font-size:13px; font-weight:700; cursor:pointer; font-family:'Nunito',sans-serif; transition:all .15s; margin-top:6px; }
        .add-variant-btn:hover { background:#f5f3ff; border-color:#7c3aed; }
              ...
              <>
                ...
                <div className="overview-grid">
                          ...
                          <tbody>
                            {orders.slice(0,6).map(o => (
                              <tr key={o.id}>
                                ...
                                <td>
                                  <span style={{ background:STATUS_BG[o.status]||'#f5f5f5', color:STATUS_COLOR[o.status]||'#555', padding:'3px 10px', borderRadius:50, fontSize:11, fontWeight:700 }}>
                  ...
                  <div className="adm-card">
                    ...
                    {pendingReviews.length > 0 && (
                      <button className="adm-btn adm-btn-green mob-only" style={{ width:'100%', borderRadius:'0 0 18px 18px', padding:'12px' }} onClick={() => handleTabChange('reviews')}>Manage All Reviews →</button>
                    )}
                  </div>
                </div>
              </>
              ...
              <div className="adm-card" style={{ background:'transparent', boxShadow:'none' }}>
                <div className="adm-card-hdr" style={{ background:'#fff', borderRadius:'18px 18px 0 0', borderBottom:'1.5px solid #f0faf3' }}>
                  ...
                  <div className="search-wrap">
                    <input className="adm-search" placeholder="🔍 Name, phone or ID..." value={orderSearch} onChange={e => setOrderSearch(e.target.value)} />
                ...
                <div className="mob-only" style={{ background:'#fff', padding:'12px 16px', borderBottom:'1.5px solid #e8ede9' }}>
                  <select className="mob-filter-select" value={orderFilter} onChange={e => setOrderFilter(e.target.value)} style={{ marginBottom:0 }}>
                  ...
                  <>
                    <div className="desk-only" style={{ overflowX:'auto', background:'#fff', borderRadius:'0 0 18px 18px' }}>
                      <table className="adm-table">
                        <thead>
                          <tr>
                            <th>#ID</th><th>Customer</th><th>Location</th><th>Items</th>
                            <th>Payment</th><th>Total</th><th>Status</th><th>Date</th><th>Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                                ...
                                <td style={{ maxWidth:180 }}>
                                  {items.slice(0,2).map((item,i) => (
                                    <div key={i} style={{ fontSize:11, color:'#555' }}>
                                      • {item.name}
                                      {item.selectedVariant && (
                                        <span style={{ background:'#ede9fe', color:'#5b21b6', fontSize:10, fontWeight:700, padding:'0 5px', borderRadius:50, marginLeft:4 }}>
                                          {item.selectedVariant}
                                        </span>
                                      )}
                                      {' '}({item.isWeightBased||item.is_weight_based ? item.weightLabel||item.weight_label : `×${item.qty}`})
                          ...
                          <td>
                            {p.has_variants && p.variants?.length > 0 ? (
                              <div style={{ display:'flex', gap:3, flexWrap:'wrap', maxWidth:160 }}>
                                ...
                                {p.variants.length > 3 && <span style={{ fontSize:10, color:'#888' }}>+{p.variants.length-3}</span>}
                              </div>
                            ) : (
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {p.has_variants && p.variants?.length > 0 && (
                            <div style={{ display:'flex', gap:3, flexWrap:'wrap', marginTop:6 }}>
                              {p.variants.map((v,i) => (
                                <span key={i} style={{ background:'#ede9fe', color:'#5b21b6', fontSize:10, fontWeight:700, padding:'2px 7px', borderRadius:50 }}>{v.label}</span>
                              ))}
                            </div>
                          )}
                        </div>
                      </div>
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', borderTop:'1.5px solid #f0faf3', paddingTop:'10px' }}>
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            >
              ...
              <div className="variant-toggle-text">
                ...
                <div className="variant-toggle-sub">e.g. Vanilla & Chocolate flavours, Orange & Mango taste</div>
              </div>
            ...
            {pForm.has_variants && (
              <div className="variants-box">
                <div className="variants-box-hdr">
                  <span className="variants-box-title">🎨 Variant Options</span>
                ...
                {(pForm.variants || []).map((v, idx) => (
                  <div key={idx} className="variant-row">
                    <input
                      ...
                      placeholder={`Option ${idx + 1} (e.g. Vanilla)`}
                    />
                    <input
                      className="variant-inp variant-price-col"
                      type="number"
                    ...
                    />
                    <button
                      className="variant-rm-btn"
                      onClick={() => removeVariant(idx)}
                      title="Remove variant"
                      disabled={pForm.variants?.length <= 1}
                    >
                      ✕
                    </button>
                  </div>
                ...
                <div className="variant-hint">
                  💡 <strong>Price override</strong> is optional — leave blank to use the base product price. Variants are shown to customers as a required selection before adding to cart.
                </div>
              </div>
            )}
            {/* ── END VARIANTS ─────────────────────────────────────────────── */}
            ...
            <Field label="Product Image">
              ...
              {uploading && (
                <div style={{ fontSize:12, color:'#1e6641', marginTop:6, fontWeight:600, display:'flex', alignItems:'center', gap:6 }}>
                  <span style={{ animation:'spin 1s linear infinite', display:'inline-block' }}>⏳</span> Uploading to Cloudinary...
                </div>
              )}
